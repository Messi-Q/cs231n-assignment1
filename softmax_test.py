...
# 时间效率比
tic = time.time()
loss_naive, grad_navie = softmax_loss_naive(W, X_dev, y_dev, 0.000005)
toc = time.time()
print('naive loss: %e compute in %fs' % (loss_naive, toc - tic))
...
# 比较梯度(用弗罗贝尼乌斯范数Frobenius norm)
grad_difference = np.linalg.norm(grad_navie - grad_vectorized, ord='fro')
